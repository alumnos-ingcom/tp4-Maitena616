...
def ingreso_entero(mensaje):
    ...
    try:
        ...
        raise IngresoIncorrecto("No es un numero entero") from err
    return entero
...
def ingreso_entero_reintento(mensaje, cantidad_reintentos=5):
    for i in range(cantidad_reintentos):
        ...
        if i == (cantidad_reintentos -1):
            raise IngresoIncorrecto("Intentos agotados") from err
    return entero

def ingreso_entero_restringido(mensaje,valor_minimo=0, valor_maximo=10):
    print(f'{mensaje}\nEntre {valor_minimo} y {valor_maximo}')
    entero = input('Numero a evaluar: ')
    
    try:
        entero = int(entero)
    except ValueError as err:
        raise IngresoIncorrecto("No esta ingresando numeros ") from err
